...
public class HypotrochoidTest1 extends JFrame implements MouseListener, KeyListener
{
	...
	public void paint(Graphics g)
	{
		...
		double	r, theta, x, y, lenbtwcenters, innerangle;
		...
		for (double angle = arcstart; angle <= arcend; angle += angleincr) {
			innerangle = Math.PI - (angle * ratioangles);		// angle between pen and line connecting centers
			// use the Law of Cosines to find the distance to the tip of the pen
			r = Math.sqrt(lenbtwcenters*lenbtwcenters + penlength*penlength
					      - 2.0*penlength*lenbtwcenters*Math.cos(innerangle));
			// use the Law of Sines to find the angle to the tip of the pen
			theta = angle - Math.asin(penlength*Math.sin(innerangle) / r);
	...
	{
		HypotrochoidTest1 app = new HypotrochoidTest1();
