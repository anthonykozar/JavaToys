/*	PolytrochoidTest0.java
	...
	This is an early test that adds a third wheel with fixed parameters to my HypotrochoidTest2
	code.  The math might not strictly be correct as the position and radius of the third wheel
	are both tied to the "pen position" of the second wheel.  This makes for some interesting 
	twisting when the pen position value is animated though!
...
public class PolytrochoidTest0 extends JFrame implements MouseListener, KeyListener
{
	...
	public PolytrochoidTest0()
	{
		super("Polytrochoid Test 0: polytrochoids in rectangular coordinates");
	...
	public void paint(Graphics g)
	{
		...
		double	x1, y1, x, y, lenbtwcenters, innerangle, innerangle2, ratioangles2, penlength2;
		...
		for (double angle = arcstart; angle <= arcend; angle += angleincr) {
			...
			innerangle2 = innerangle + (innerangle * ratioangles2);
			// calculate coordinates of point relative to the origin
			// NOTE: penlength is probably not correct here ?
			x1 = lenbtwcenters * Math.cos(angle) + penlength * Math.cos(innerangle) + penlength2 * Math.cos(innerangle2);
			y1 = lenbtwcenters * Math.sin(angle) + penlength * Math.sin(innerangle) + penlength2 * Math.sin(innerangle2);
	...
	{
		PolytrochoidTest0 app = new PolytrochoidTest0();
